...
namespace OpenSMOKE
{
	...
	void PressureLogarithmicRateExpression::Setup(std::vector<double> coefficients_)
	{
		...
		N = static_cast<unsigned int>(p_atm.size());
		lnA_.resize(N);
		Beta_.resize(N);
		E_over_R_.resize(N);
		p_.resize(N);
	...
	void PressureLogarithmicRateExpression::WriteCHEMKINReactionData(std::stringstream& reaction_data, const double conversion_factor_A) const
	{
		for (unsigned int j=0;j<N;j++)
		{
			for (unsigned int k = 0; k < p_[j].size(); k++)
			{
				reaction_data << " PLOG /  ";
				reaction_data << std::showpoint << std::setw(16) << std::scientific << std::left << p_[j][k] / 101325.;
				reaction_data << std::showpoint << std::setw(16) << std::scientific << std::left << std::exp(lnA_[j][k]) / conversion_factor_A;
				reaction_data << std::showpoint << std::setw(16) << std::scientific << std::left << Beta_[j][k];
				reaction_data << std::showpoint << std::setw(16) << std::scientific << std::left << E_over_R_[j][k] * PhysicalConstants::R_J_kmol / Conversions::J_from_kcal;
				reaction_data << " /";
