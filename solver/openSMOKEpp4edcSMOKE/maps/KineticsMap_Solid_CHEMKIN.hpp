...
namespace OpenSMOKE
{ 
	KineticsMap_Solid_CHEMKIN::KineticsMap_Solid_CHEMKIN(ThermodynamicsMap_Solid_CHEMKIN& thermo, boost::property_tree::ptree& ptree, const unsigned int target) :
	...
	KineticsMap_Solid_CHEMKIN::KineticsMap_Solid_CHEMKIN(ThermodynamicsMap_Solid_CHEMKIN& thermo, boost::property_tree::ptree& ptree, const std::string target) :
	thermodynamics_(thermo)
	{
		ImportSpeciesFromXMLFile(ptree);
		ImportCoefficientsFromXMLFile(ptree, target);
	...
	void KineticsMap_Solid_CHEMKIN::ImportCoefficientsFromXMLFile(boost::property_tree::ptree& ptree)
	{
		ErrorMessage("void KineticsMap_Solid_CHEMKIN::ImportCoefficientsFromXMLFile(boost::property_tree::ptree& ptree)", "The solid kinetic map require the user specifies tha material name.");
	}

	void KineticsMap_Solid_CHEMKIN::ImportCoefficientsFromXMLFile(boost::property_tree::ptree& ptree, const std::string target_material_name)
	{
		...
		if (kinetics_type != "OpenSMOKE" || kinetics_version != "01-02-2014")
			ErrorMessage("void KineticsMap_Solid_CHEMKIN::ImportCoefficientsFromXMLFile(boost::property_tree::ptree& ptree)", "The current kinetic scheme is not supported.");
		...
		if (target_material_index == 0)
			ErrorMessage("void KineticsMap_Solid_CHEMKIN::ImportCoefficientsFromXMLFile(boost::property_tree::ptree& ptree)", "The requested solid material is not available. Please check the name.");
		else
			ImportCoefficientsFromXMLFile(ptree, target_material_index);
	}
 
	void KineticsMap_Solid_CHEMKIN::ImportCoefficientsFromXMLFile(boost::property_tree::ptree& ptree, const unsigned int target_material_index)
	{
				...
				if (target_material_index == subtree.get<unsigned int>("<xmlattr>.index") )
				{
					...
					// Stoichiometry
					{
						...
						if (stoichiometry_type != "OpenSMOKE" || stoichiometry_version != "01-02-2014")
							ErrorMessage("void KineticsMap_Solid_CHEMKIN::ImportCoefficientsFromXMLFile(boost::property_tree::ptree& ptree)", "The current stoichiometric data are not supported.");
	...
	void KineticsMap_Solid_CHEMKIN::ImportSpeciesFromXMLFile(boost::property_tree::ptree& ptree)
	{
		try
		{
