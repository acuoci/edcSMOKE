...
namespace OpenSMOKE
{ 
	ThermodynamicsMap_Solid_CHEMKIN::ThermodynamicsMap_Solid_CHEMKIN(boost::property_tree::ptree& ptree)
	{
	...
	void ThermodynamicsMap_Solid_CHEMKIN::ImportSpeciesFromXMLFile(boost::property_tree::ptree& ptree)
	{
		// Names of species
		{
			try
		...
		// Reading all the available materials
		{
			...
			BOOST_FOREACH( boost::property_tree::ptree::value_type const& node, ptree.get_child( "opensmoke" ) ) 
			{
				...
				if( node.first == "MaterialDescription" ) 
				{
					...
					{
						stream >> matrix_names_solid_species_[k][i];
						stream >> matrix_indices_solid_species_[k][i];
					}
	...
	void ThermodynamicsMap_Solid_CHEMKIN::ImportCoefficientsFromXMLFile(boost::property_tree::ptree& ptree)
	{
