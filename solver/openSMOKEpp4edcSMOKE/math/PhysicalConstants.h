...
namespace PhysicalConstants
{
	...
	const double alphaCollision = std::sqrt(8.*pi*kBoltzmann)*Nav_kmol;	// collision rate constant (bimolecular reactions) [#/kmol*m/s*sqrt(kg/K)]

	enum TAG_REACTION { REACTION_SIMPLE, REACTION_THIRDBODY, REACTION_LINDEMANN_FALLOFF, REACTION_SRI_FALLOFF, REACTION_TROE_FALLOFF, REACTION_LINDEMANN_CABR, REACTION_SRI_CABR, REACTION_TROE_CABR, REACTION_CHEBYSHEV, REACTION_EXTENDEDFALLOFF, REACTION_REDUCEDPRESSUREBASEDMIXRULE };
	...
	const double MAX_MW_THERMALDIFFUSION_RATIOS = 5;

	const double ZROTA = 0.5*std::pow(boost::math::constants::pi<double>(), 1.5);
	const double ZROTB = 2.0+0.25*boost::math::pow<2>(boost::math::constants::pi<double>());
	const double ZROTC = std::pow(boost::math::constants::pi<double>(), 1.5);

	enum OpenSMOKE_GasMixture_Viscosity_Model { OPENSMOKE_GASMIXTURE_VISCOSITYMODEL_WILKE, OPENSMOKE_GASMIXTURE_VISCOSITYMODEL_HERNING, OPENSMOKE_GASMIXTURE_VISCOSITYMODEL_MATHUR_SAXENA };
}
