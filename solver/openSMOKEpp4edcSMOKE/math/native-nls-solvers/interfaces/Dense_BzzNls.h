...
#ifndef OpenSMOKE_Solve_Dense_BzzNls_H
#define OpenSMOKE_Solve_Dense_BzzNls_H
...
namespace NlsSMOKE
{
	...
	int Solve_Dense_BzzNls(	Object* object, BzzNonLinearSystemObject& nls_object, 
							const NlsSMOKE::NonLinearSolver_Parameters& parameters,
							const double t, Eigen::VectorXd& y, Eigen::VectorXd& f)
	{
		std::cout << "Dense NLS solution (BzzNls)..." << std::endl;
		...
		nls_object(yInitial, &nls_system);
		...
		if (status >= 0)
		{
			...
			// Get solution and residuals
			BzzVector ybzz(neq), fbzz(neq);
			nls_object.GetSolution(&ybzz, &fbzz);

			// Move solution from BzzMath to Eigen vectors
			y.resize(neq);
			f.resize(neq);
			for (unsigned int i = 1; i <= neq; i++)
			{
			...
			object->CorrectedUnknownsVector(ybzz.GetHandle());
		}
		else
		{
...
#endif // OpenSMOKE_Solve_Dense_BzzNls_H
