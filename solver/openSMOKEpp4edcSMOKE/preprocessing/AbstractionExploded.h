...
namespace OpenSMOKE
{
	...
	*/
	...
	public:
		...
		* Checks if one or more exploded abstraction reactions are already present in the kinetic mechanism
		*/
		unsigned int RemoveExistingReversibleReactions(const std::vector<std::string>& reactant_species, const std::vector<std::string>& product_species);
		...
		*/
		unsigned int type_H() const { return type_H_; }
	...
	public:
		...
		std::vector<std::string> species_Rp_;		//!< R' species: R+R'H=>R'+RH
		std::vector<std::string> species_Rp_nu_;	//!< R' species stoichiometryic coefficients: R+R'H=>R'+RH
