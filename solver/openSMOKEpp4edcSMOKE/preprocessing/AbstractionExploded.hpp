...
namespace OpenSMOKE
{
		...
		const double n_H, const unsigned int type_H)
	{
		...
		n_ = static_cast<unsigned int>(species_R_.size());
		A_.resize(n_);
		Beta_.resize(n_);
		E_.resize(n_);
		is_dummy_.resize(n_);
		...
		{
			species_R_length[i] = static_cast<int>(species_R_[i].length());
			species_RH_length[i] = static_cast<int>(species_RH_[i].length());
		}
												...
												const double Eref, const double alpha, const double Tref)
	{
		...
		for (unsigned int j = 0; j < species_Rp_.size(); j++)
		{
			...
			{
				index_Rprime = static_cast<int>(std::distance(abstractors.begin(), iRprime));
			}
		}
		...
		for (unsigned int i = 0; i < n_; i++)
		{
			...
			const unsigned int index_R = static_cast<unsigned int>(std::distance(abstractors.begin(), iR));
