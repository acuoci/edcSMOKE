...
namespace OpenSMOKE
{
	...
	{
		unsigned int last_line_of_abstractors = static_cast<unsigned int>(kinetics.good_lines().size());
		unsigned int last_line_of_corrections = static_cast<unsigned int>(kinetics.good_lines().size());
		unsigned int last_line_of_abstractions = static_cast<unsigned int>(kinetics.good_lines().size());
	...
	bool AbstractionReactions::ExtractTableAbstractors()
	{
		for (unsigned int j = 0; j < lines_abstractors_.size(); j++)
		{
			...
			for (tokenizer::iterator tok_iter = tokens.begin(); tok_iter != tokens.end(); ++tok_iter)
			{
				...
				{
					const double A = boost::lexical_cast<double>(*tok_iter);
					if (A<=0.)
					{
						std::cout << "Reading ABSTRACTORS section. Error in the following line:" << std::endl;
						std::cout << lines_abstractors_[j] << std::endl;
						std::cout << "The frequency factor must be strictly positive" << std::endl;
						return false;
					}
				...
				{
					Beta_.push_back(boost::lexical_cast<double>(*tok_iter));
				}
				else if (count == 4)
				{
					const double E = boost::lexical_cast<double>(*tok_iter);
					if (E<0.)
					{
						std::cout << "Reading ABSTRACTORS section. Error in the following line:" << std::endl;
						std::cout << lines_abstractors_[j] << std::endl;
						std::cout << "The activation energy cannot be negative" << std::endl;
						return false;
					}
		...
		for (unsigned int j = 0; j < lines_abstractions_.size(); j++)
		{
			...
			if (element1 == "TREF")
			{
				...
				if (Tref_ <= 0.)
				{
					std::cout << "Reading ABSTRACTIONS section. Error in the following line:" << std::endl;
					std::cout << lines_abstractions_[j] << std::endl;
					std::cout << "Reference temperature TREF must be strictly positive" << std::endl;
			...
			else if (element1 == "EREF")
			{
				...
				if (Eref_ <= 0.)
				{
					std::cout << "Reading ABSTRACTIONS section. Error in the following line:" << std::endl;
					std::cout << lines_abstractions_[j] << std::endl;
					std::cout << "Reference activation energy EREF must be strictly positive" << std::endl;
	...
	int AbstractionReactions::ParseReaction(ReactionPolicy_CHEMKIN& reaction, const std::vector<std::string>& map_of_species)
	{
			...
			std::find(product_species.begin(), product_species.end(), "RH") != product_species.end())
		{
			if (reactant_species.size() == 2 && product_species.size() >=2 )
			{
				...
				std::vector<std::string> Rp_nu;
				for (unsigned int i = 0; i < reaction.product_nu().size(); i++)
					if (product_species[i] != "RH")
					{
						Rp.push_back(product_species[i]);

						if (reaction.product_nu()[i] == 1.)			Rp_nu.push_back("");
						else if (reaction.product_nu()[i] == 2.)	Rp_nu.push_back("2");
						else if (reaction.product_nu()[i] == 0.5)	Rp_nu.push_back("0.5");
						else										Rp_nu.push_back(std::to_string(reaction.product_nu()[i]));
					}

				const double n_H = reaction.A()*1e3;
				const int type_H = static_cast<int>(std::round(reaction.E_over_R()*PhysicalConstants::R_cal_mol));

				exploded_.push_back(AbstractionExploded(abstractors_, RpH, Rp, Rp_nu, abstracted_, n_H, type_H));
	...
	unsigned int AbstractionReactions::RemoveExistingReactions(ReactionPolicy_CHEMKIN& reaction, const std::vector<std::string>& map_of_species)
	{
...

		{
			...
			if (reaction.IsReversible() == true)
			{
				for (unsigned int j = 0; j < exploded_.size(); j++)
					n_existing += exploded_[j].RemoveExistingReversibleReactions(reactant_species, product_species);
			}
		}
	...
	void AbstractionReactions::ExplodeReactions()
	{
		...
		for (unsigned int i = 0; i < abstractors_.size(); i++)
		{
			...
			for (unsigned int j = 0; j < exploded_.size(); j++)
				...
				{
					corrections_Er_[i] = corrections_E_[exploded_[j].type_H()];
