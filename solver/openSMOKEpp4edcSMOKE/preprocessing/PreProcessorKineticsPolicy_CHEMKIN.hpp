...
namespace OpenSMOKE
{
	...
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::ReadFromASCIIFile(const std::string file_name)
	{
...

		{
			...
			species_classes_ = new SpeciesClasses(myKinetics->species_class_names(), myKinetics->species_class_lines_abs());
			...
			// Species section 
			{
				...
				for (unsigned int i=iSpeciesLines.front();i<=last_line_of_species;i++)
				{
					...
					species_lines.push_back(i);
				}
				species_lines.erase(species_lines.begin()); // Remove the index of the word "Species" or alternative name
				// species_lines.erase(species_lines.end());   // Remove the index of the word "end" or alternative name to be investigated if needed
				...
				// Checking if a species appears more than once
				{
	...
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::KineticsFromASCIIFile(AtomicCompositionTable& atomicComposition, std::ostream& flog)
	{
		...
		PhysicalConstants::UNITS_REACTION a_units = PhysicalConstants::UNITS_MOLES;
		{
			...
			for (tokenizer_blank::iterator tok_iter = boost::next(tokens.begin()); tok_iter != tokens.end(); ++tok_iter)
			{
				std::string tag = *tok_iter;
																								//AL
				if (tag == "CAL" || tag == "CAL/MOLE" || tag == "cal" || tag == "cal/mole" || tag.find("MAXSP")!=std::string::npos)
		...
		for (unsigned int j=0;j<iEndReactionLine;j++)
		{
			...
			if (found!=std::string::npos && myKinetics->good_lines()[j].find("MAXSP")==std::string::npos)
			{
		...
		// Species classes (if active)
		/*
		if(species_classes_->is_active() == true)
		{	
			for(unsigned int j = 0; j < species_lines.size(); j++)
				species_classes_->filter_species(j, myKinetics->indices_of_good_lines()[species_lines[j]]);
		}
		*/
		if(species_classes_->is_active() == true)
		{	
			species_classes_->filter_species_nobili(names_species_, myKinetics->firstspecies_names());
		}
		...
		if (abstractions_->is_active() == true)
		{
			...
			{
				unsigned int current_strong_comment_index = 0;
				...
				for (unsigned int j = 0; j < myKinetics->good_lines().size(); j++)
				{
					// Insert strong comment lines in the right place
					if (myKinetics->number_of_strong_comment_lines() > 0)
					{
						const unsigned int current_line_index = myKinetics->indices_of_good_lines()[j];
						for (unsigned int i=current_strong_comment_index;i<myKinetics->number_of_strong_comment_lines();i++)
						{
							if (myKinetics->indices_of_strong_comment_lines()[i] < current_line_index)
							{
								fExploded << myKinetics->strong_comment_lines()[i] << std::endl;
								current_strong_comment_index++;
							}
							else
							{
								break;
					...
					// Check if reaction has to be exploded
	...
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::WriteXMLFile(std::stringstream& fOutput) const
	{
		...
		// Species Classes if any
		if(species_classes_->is_active() == true)
		{
			species_classes_->WriteXMLFile(fOutput);
			species_classes_->Summary(std::cout);
		}
