...
namespace OpenSMOKE
{
	...
	bool PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::KineticsFromASCIIFile(	AtomicCompositionTable& atomicComposition, const PreProcessor& preprocessor_kinetics, std::ostream& flog)
	{
		...
		unsigned int total_number_of_reactions = 0; 
		...
		for(unsigned int k=0;k<number_of_materials_;k++)
		{
			...
			total_number_of_reactions += number_of_reactions;	
				...
				for (unsigned int i=0; i<reactions_[k].size(); i++)
					for (unsigned int j=i+1; j<reactions_[k].size(); j++)
					{
						if ( OpenSMOKE_Utilities::compare_vectors( reactions_[k][i].reactant_nu_indices(), reactions_[k][j].reactant_nu_indices() ) == true
							&& OpenSMOKE_Utilities::compare_vectors(reactions_[k][i].reactant_nu(), reactions_[k][j].reactant_nu()) == true)
						{
							if ( OpenSMOKE_Utilities::compare_vectors( reactions_[k][i].product_nu_indices(), reactions_[k][j].product_nu_indices() ) == true
								&& OpenSMOKE_Utilities::compare_vectors(reactions_[k][i].product_nu(), reactions_[k][j].product_nu()) == true)
							{
		...
		all_reactions_.resize(total_number_of_reactions);
		all_reaction_lines.resize(total_number_of_reactions);
		
		int k = 0;
		for (unsigned int kk = 0; kk < number_of_materials_; kk++)
		{
			for (unsigned int kkk = 0; kkk < reactions_[kk].size(); kkk++)
			{
				all_reactions_[k] = reactions_[kk][kkk];
				all_reaction_lines[k] = reaction_lines[kk][kkk];
				k++;
			}
		}
		return true;
