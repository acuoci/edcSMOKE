...
|   License                                                               |
|                                                                         |
|   Copyright(C) 2012-2023  Alberto Cuoci                                 |
...
//		Omega11jk(T);
	...
	/*
	// This was the original function before the correction suggested by Bertrand Naud
	template<typename Species>
	...
	*/
	...
	void PreProcessorSpeciesPolicy_CHEMKIN_WithTransport<Species>::BinaryDiffusionCoefficients(const double T, const double P_bar)
	{
		// The matrix is symmetric and only the upper half is calculated
		// The main diagonal is not calculated here (it corresponds to the self diffusion
		// coefficients), becuase it is calculated by a different function
		...
		for(unsigned int j=1;j<=this->NC;j++)
			for(unsigned int k =j;k<=this->NC;k++)
			{
				//Djk[k][j] = Djk[j][k] = coeff_Djk[j][k] * T_P / CollisionIntegral11(T/UncorrectedLennardJonesPotential(j,k),deltajkStar[j][k]);

				// Comment (Alberto Cuoci): in my opinion this version is more correct. However the results obtained with the version reported 
				// above are much closer to what is calculated by CHEMKIN. I have no explanation about this]
				
				//double epsylonjk_over_kb_, sigmajk_;
				//LennardJonesPotentialAndCollisionDiameter(j, k, epsylonjk_over_kb_, sigmajk_);
				//Djk[k][j] = Djk[j][k] = coeff_Djk[j][k] * T_P / CollisionIntegral11(T/epsylonjk_over_kb_,deltajkStar[j][k]);

				// Answer (Bertrand Naud): This is because Chemkin includes a correction term proposed by Marrero and Mason (1972)

				const double w1 = this->MW[j];
				const double w2 = this->MW[k];
				const double sumw = w1+w2;
				const double sig1 = sigma[j];
				const double sig2 = sigma[k];
				const double sig12 = 0.5*(sig1+sig2);

				const double tr11 = T/epsylon_over_kb[j];
				const double tr22 = T/epsylon_over_kb[k];
				const double tr12 = T/std::sqrt(epsylon_over_kb[j]*epsylon_over_kb[k]);

				const double om2f11 = CollisionIntegral22(tr11,deltajkStar[j][k]);
				const double om2f22 = CollisionIntegral22(tr22,deltajkStar[j][k]);
				const double om1f12 = CollisionIntegral11(tr12,deltajkStar[j][k]);
				const double om2f12 = CollisionIntegral22(tr12,deltajkStar[j][k]);
				...
				if (tr12 <= 5.)
				{
					bst12 = 1.36  + (-0.223 + (0.0613 - 0.00554*tr12)*tr12)*tr12;
					cst12 = 0.823 + (0.0128 + (0.0112 - 0.00193*tr12)*tr12)*tr12;
				}

				double h = boost::math::pow<2>(sig1/sig2) * std::sqrt(2.*w2/sumw) * 2.*w1*w1/(sumw*w2);
				const double p1 = h * om2f11/om1f12;

				h = boost::math::pow<2>(sig2/sig1) * std::sqrt(2.*w1/sumw) * 2.*w2*w2/(sumw*w1);
				const double p2 = h * om2f22/om1f12;

				const double p12 = 15.*boost::math::pow<2>((w1 - w2)/sumw) + 8.*w1*w2*ast12/(sumw*sumw);

				h = 2./(w2*sumw) * std::sqrt(2.*w2/sumw) * om2f11/om1f12 * boost::math::pow<2>(sig1/sig12);
				const double q1 = ((2.5 - 1.2*bst12)*w1*w1 + 3.*w2*w2 + 1.6*w1*w2*ast12)*h;

				h = 2./(w1*sumw) * std::sqrt(2.*w1/sumw) * om2f22/om1f12 * boost::math::pow<2>(sig2/sig12);
				const double q2 = ((2.5 - 1.2*bst12)*w2*w2 + 3.*w1*w1 + 1.6*w1*w2*ast12)*h;

				h = boost::math::pow<2>((w1 - w2)/sumw) * (2.5 - 1.2*bst12)*15.  + 4.*w1*w2*ast12/(sumw*sumw) * (11. - 2.4*bst12);
				const double q12 = h + 1.6*sumw*om2f11*om2f22 / (std::sqrt(w1*w2)*om1f12*om1f12) * sig1*sig1*sig2*sig2/boost::math::pow<4>(sig12);

				const double corr_Marrero = 1. + (boost::math::pow<2>(6.*cst12 - 5.)/10.) * (p1 + p2 + p12) / (q1 + q2 + q12);
				
				double epsylonjk_over_kb_, sigmajk_;
				LennardJonesPotentialAndCollisionDiameter(j, k, epsylonjk_over_kb_, sigmajk_);
				Djk[k][j] = Djk[j][k] = corr_Marrero * coeff_Djk[j][k] * T_P / CollisionIntegral11(T/epsylonjk_over_kb_,deltajkStar[j][k]);
			}
	}
			...
			if (is_teta_feasible == false)
			{
				...
				if (NT>0)
				{
					...
					for (int i=0;i<nPoints;i++)
					{
						...
						// Polynomial with respect to T (scaled at 1000K)
						X(i,0)=1.;
						X(i,1)=T/1.e3;
						X(i,2)=T*T/1.e6;
						X(i,3)=T*T*T/1.e9;
					...
					// Polynomial with respect to T (scaled)
					for(unsigned int i=0;i<this->NC*NT;i++)
					{
						fittingTetaBinary(1,i) /= 1.e3;
						fittingTetaBinary(2,i) /= 1.e6;
						fittingTetaBinary(3,i) /= 1.e9;
					}
	...
	void PreProcessorSpeciesPolicy_CHEMKIN_WithTransport<Species>::WriteBinaryDiffusivityFittingCoefficients(std::ostream &fOutput) const
	{
		...
		int count=1;
		for (unsigned int i=1;i<=this->NC;i++)
			for (unsigned int k=i;k<=this->NC;k++)
