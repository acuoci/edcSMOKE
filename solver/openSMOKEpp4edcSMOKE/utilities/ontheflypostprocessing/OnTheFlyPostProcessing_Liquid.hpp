...
namespace OpenSMOKE
{
	...
	OnTheFlyPostProcessing_Liquid::OnTheFlyPostProcessing_Liquid(OpenSMOKE::ThermodynamicsMap_Liquid_CHEMKIN& thermodynamicsMap,
	...
	void OnTheFlyPostProcessing_Liquid::SetupFromDictionary(OpenSMOKE::OpenSMOKE_Dictionary& dictionary)
	{
	...
	void OnTheFlyPostProcessing_Liquid::MemoryAllocation()
	{
		// Basic variables
		NSG_ = thermodynamicsMap_.number_of_gas_species();
		NSL_ = thermodynamicsMap_.number_of_liquid_species();
		NS_ = NSG_ + NSL_;
		NR_ = kineticsMap_.NumberOfReactions();

		if (is_active_ == true)
		{
			OpenSMOKE::ChangeDimensions(NSG_, &xG_, true);
			OpenSMOKE::ChangeDimensions(NSG_, &cGas_, true);			
			OpenSMOKE::ChangeDimensions(NSG_, &Rg_, true);

			OpenSMOKE::ChangeDimensions(NSL_, &xL_, true);
			OpenSMOKE::ChangeDimensions(NSL_, &cLiq_, true);
			OpenSMOKE::ChangeDimensions(NSL_, &Rl_, true);

			OpenSMOKE::ChangeDimensions(NS_, &R_, true);
		}
	}

	void OnTheFlyPostProcessing_Liquid::CloseOutputFiles()
	{
	...
	void OnTheFlyPostProcessing_Liquid::PrepareOutputFiles(const std::vector<std::string>& additional)
	{
		// Output file
	...
	void OnTheFlyPostProcessing_Liquid::PrepareOutputFiles()
	{
		std::vector<std::string> additional(0);
		PrepareOutputFiles(additional);
	}	

	void OnTheFlyPostProcessing_Liquid::WriteOnFile(const double t, const double x, const double y, const double z, const double T, const double P_Pa, 
		const double rho, const OpenSMOKE::OpenSMOKEVectorDouble& omegaG, const OpenSMOKE::OpenSMOKEVectorDouble& omegaL, const std::vector<double>& additional)
	{
		double QR = 0;
		...
		// Kinetic data
		{
			...
			// Gas Mixture 
			double MW, cTot;
			thermodynamicsMap_.MoleFractions_From_MassFractions(xG_.GetHandle(), MW, omegaG.GetHandle());
			cTot = P_Pa / (PhysicalConstants::R_J_kmol * T);
			Product(cTot, xG_, &cGas_);
			
			// Liquid Mixture 
			thermodynamicsMap_.MoleFractions_From_MassFractions(xL_.GetHandle(), MW, omegaL.GetHandle());
			cTot = rho / MW;
			Product(cTot, xL_, &cLiq_);

			kineticsMap_.SetTemperature(T);
			kineticsMap_.SetPressure(P_Pa);
			kineticsMap_.ReactionRates(cGas_.GetHandle(), cLiq_.GetHandle());
			kineticsMap_.FormationRates(Rg_.GetHandle(), Rl_.GetHandle());
			for (int i = 1; i <= NSG_; i++)
				R_[i] = Rg_[i];
			for (int i = 1; i <= NSL_; i++)
				R_[i + NSG_] = Rl_[i];
			
			QR = kineticsMap_.HeatRelease(Rg_.GetHandle(), Rl_.GetHandle());
		}
		...
		if (class_grouping_ == true)
		{
			...
			{
				sum_x[class_grouping_indices_families_[j]] += xL_[class_grouping_indices_species_[j] + 1];
				sum_omega[class_grouping_indices_families_[j]] += omegaL[class_grouping_indices_species_[j] + 1];
			}
	...
	void OnTheFlyPostProcessing_Liquid::WriteOnFile(const double t, const double x, const double y, const double z, const double T, const double P_Pa, 
		const double rho, const OpenSMOKE::OpenSMOKEVectorDouble& omegaG, const OpenSMOKE::OpenSMOKEVectorDouble& omegaL)
	{
		std::vector<double> additional(0);
		WriteOnFile(t, x, y, z, T, P_Pa, rho, omegaG, omegaL, additional);
	}
}
