...
namespace OpenSMOKE
{
	...
	void HMOM::SetupFromDictionary(OpenSMOKE::OpenSMOKE_Dictionary& dictionary)
	{
		...
		if (dictionary.CheckOption("@PAHSum") == true)
		{
			bool flag;
			dictionary.ReadBool("@PAHSum", flag);
			SetPAHSum(flag);
		}
	...
	void HMOM::SetPAH(const std::vector<std::string> pah_species)
	{
		...
		dimerization_rate_.resize(pah_species_.size());
	}
	...
	void HMOM::SetPAHSum(const bool flag)
	{
		pah_sum_ = flag;
	}
	...
	void HMOM::DimerConcentration()
	{
		...
		if (pah_sum_ == false)
		{
			for (unsigned int j=0;j<conc_PAH_.size();j++)
			{
				const double aromatics_conc = conc_PAH_(j)*1.e6;
				dimerization_rate_(j) = 0.5* KfmPAH * std::pow(aromatics_conc, 2.) * sticking_coefficient_;	// [mol/m3/s]
			}
			const double delta = std::pow(betaC, 2.) + 4.0 * betaN_ * dimerization_rate_.sum() * std::pow(AvogadroNumber, 2.0);
			conc_DIMER_ = (std::sqrt(delta) - betaC) / (2.0*betaN_*AvogadroNumber);
		}
		else
		{
			const double aromatics_conc = conc_PAH_.sum()*1.e6;
			const double dimerization_rate = 0.5* KfmPAH * std::pow(aromatics_conc, 2.) * sticking_coefficient_;	// [mol/m3/s]
			const double delta = std::pow(betaC, 2.) + 4.0 * betaN_ * dimerization_rate * std::pow(AvogadroNumber, 2.0);
			conc_DIMER_ = (std::sqrt(delta) - betaC) / (2.0*betaN_*AvogadroNumber);

			for (unsigned int j=0;j<conc_PAH_.size();j++)
				dimerization_rate_(j) = dimerization_rate * ( conc_PAH_(j)/conc_PAH_.sum() );
		}
	}

	double HMOM::PAHConsumptionRate(const unsigned int j) const
	{
		return 2.*dimerization_rate_(j)*AvogadroNumber;		// [mol/m3/s]
	}
